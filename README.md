...
Check if each name in the list present in the tags ARTIST (TPE1 for mp3), or ALBUMARTIST (TPE2 for mp3) corresponds to an existing directory. The separator character of the lists is '/' or ';' ( '&' generate an error ). The artist name 'Anonymous' is not tested.

Check if the tag COMPOSER (TCOM for mp3) corresponds to an existing directory. No separator character is used ( bug Jellyfin ?).
